...
public class kEMCSz2 {
	...
	public kEMCSz2(double dc, int pnum) {
		this.dc = dc;
		this.pnum = pnum;
	...
	public void init(String input) {
		 ...
		 * 
		 * @Override public int compare(Point p1, Point p2) { if (p1 != null && p2 !=
		 * null) { if (q.dis(p1) < q.dis(p2)) { return 1; } else if (q.dis(p1) >
		 * q.dis(p2)) { return -1; } else { return 0; } } return 0; } });
		 */
...
//找到所有的圆
	public void method(double dc) throws FileNotFoundException {
		PrintStream ps = new PrintStream("result2.txt");
		System.setOut(ps);
		...
		for (int i = 0; i < plist.size(); i++) {
			...
			if (qp1 > tempd) {
				...
				System.out.println("i point: x: "+p1.x+"y: "+p1.y+" i dis q: "+qp1+" izvalue: "+p1.zval+" iupperzvalue: "+p1.pupperzval);
				...
				for (int j = i + 1; j < ppostionend; j++) {
					...
					if (qp2 > tempd) {
						...
						jj++;
						System.out.println("j point: x: "+p2.x+"y: "+p2.y+" j dis q: "+qp1+" jzvalue: "+p2.zval);
						pointpairnum ++;
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		kEMCSz2 sc = new kEMCSz2(dc, pnum);
		sc.init("f.txt");
